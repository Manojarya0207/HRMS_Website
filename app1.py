...
from flask import Flask, Response, current_app, render_template, request, redirect, url_for, session, flash, jsonify, abort
...
# Wiki Category Image upload
WIKI_CAT_FOLDER = 'static/wikiCatImg'
app.config['WIKI_CAT_FOLDER'] = WIKI_CAT_FOLDER
if not os.path.exists(WIKI_CAT_FOLDER):
    os.makedirs(WIKI_CAT_FOLDER)
...
@app.route('/employee/expense', methods=['GET', 'POST'])
def expense():
    if 'user_id' not in session:
        return redirect(url_for('login'))
...
@app.route('/admin/expense/generate_report/<int:emp_id>')
def generate_expense_report(emp_id):
    ...
    # Load template
    template_path = 'Expense-Details.xlsx'
...
@app.route('/admin/expense/report')
def generate_expense_report_form():
    ...
    if emp_id:
        return redirect(url_for('generate_expense_report', emp_id=int(emp_id)))
...
# ------ Sub Expense types ----------------------------------
@app.route('/admin/sub_expense_types', methods=['GET', 'POST'])
...
def delete_sub_expense_type(sub_et_id):   
    ...
    return redirect(url_for('admin_sub_expense_types'))

@app.route('/api/sub_expense_types/<int:expense_type_id>')
