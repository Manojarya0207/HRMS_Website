...
@projects_bp.route('/admin/project/<int:project_id>/kanban')
def admin_project_kanban(project_id):
    if 'user_id' not in session or session['emp_type'] != 'admin':
        return redirect(url_for('auth.login'))
    ...
    tasks = db.get_tasks_by_project(project_id)
    return render_template('kanban_board.html', project=project, tasks=tasks, is_admin=True)
